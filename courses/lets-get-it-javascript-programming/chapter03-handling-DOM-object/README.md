...
- [마무리 요약](#마무리-요약)
  - [순서도 그리기](#순서도-그리기)
  - [대화 상자 띄우기](#대화-상자-띄우기)
  - [HTML 태그 선택하기](#html-태그-선택하기)
  - [태그에 이벤트 달기](#태그에-이벤트-달기)
  - [순서도 최적화하기](#순서도-최적화하기)
...
프로그래밍에서 가장 중요한 것은 코드를 작성하기 전에 올바른 순서도를 만드는 것입니다. 물론 한 번에 순서도를 완성할 수는 없고 코딩하면서 계속 수정해야 합니다. 하지만 다음 원칙을 지키면서 순서도를 설계한다면 수정하는 횟수와 전체 절차 수를 최소화할 수 있습니다(코딩 속도가 빨라집니다).
...
순서도를 만들 때 사용자의 이벤트(버튼 클릭, 입력창 글자 입력 등)가 필요한 곳에서 순서도를 끊어야 함을 잊지 마세요!

### 대화 상자 띄우기

웹 브라우저에 대화 상자는 다음 세 가지로 띄울 수 있습니다.
...
document.querySelector로는 하나의 태그만 선택할 수 있고, document.querySelectorAll로는 여러 개의 태그를 선택할 수 있습니다.
...
단순한 태그일 때 선택자는 태그의 이름이 됩니다. 예를 들어, 버튼 태그는 button이 선택자입니다. 하나의 태그만 콕 찝어서 선택하고 싶을 때는 id 속성을 사용합니다. 선택자로는 #아이디를 사용합니다. 여러 개의 태그를 동시에 선택하고 싶을 때는 class 속성을, 선택자로는 .클래스를 사용합니다.
...
어떤 태그 안에 들어 있는 다른 태그를 선택하고 싶다면 선택자 사이를 한 칸 띄면 됩니다. 그러면 앞 선택자 안에 들어 있는 태그가 선택됩니다.
...
태그를 선택한 후에 addEventListener 메서드를 사용해 이벤트를 연결합니다. 첫 번째 인수로 이벤트 이름을 넣고, 두 번째 인수로 리스너 함수를 넣습니다. 리스너 함수는 이벤트가 발생할 때 실행되는 함수라고 생각하면 됩니다.

```js
태그.addEventListener('이벤트 이름', 리스너함수);
```

리스너 함수의 매개변수로 event 객체를 제공해서 이벤트와 관련된 정보를 얻을 수 있습니다. 예를 들어, input 태그에 입력된 값을 가져오려면 event.target.value를 넣으면 됩니다. 여기서 event.target은 이벤트가 발생한 대상 태그를 가리킵니다.
...
입력창에 입력된 값은 value 속성으로 가져옵니다. value에 값을 대입하면 대입한 값으로 변경됩니다.
...
입력 태그(input, select, textarea 등)가 아닌 일반 태그들의 내부 값을 가져올 때는 value가 아니라 textContent 속성을 사용합니다.
...
입력창이나 버튼의 경우 focus 메서드를 호출하면 해당 태그가 하이라이트됩니다.
...
여러 개의 if 문을 하나로 합치려면 진리표를 활용합니다. 두 if 문의 관계가 OR(||, 또는)인지 AND(&&, 그리고)인지에 따라 진리표가 달라집니다.
